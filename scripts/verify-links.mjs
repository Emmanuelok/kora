...
for(const link of staticLinks){const path=link.split('?')[0];assert(pages.has(path)||(path.startsWith('/product/')&&products.some(p=>path==='/product/'+p.id))||path.startsWith('/help/')||path==='/help'||path==='/catalogue-inventory.csv',`Unimplemented link ${link}`)}
